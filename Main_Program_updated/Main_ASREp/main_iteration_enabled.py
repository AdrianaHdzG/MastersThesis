...
sys.path.append("C:\GITrepo\Main_Program_updated\ASREpy-main")
...
resultname = "results_LTS_xBoverHL.mat"

# IF 'True', makes a series of mat files wih name 'filename' seen below, else use 'False'
makeIndividualMATfiles = False

for iterPara in range(resultlength):            # Loops from 0 to resultlength - 1
    if iterPara != 0:                           # If not the first iteration (to keep standard case in there)
        building_offset = iterPara  / scale       # Assign the new testing variable

        # Update x-coordinates for the building [m]
        building_coords = building_offset + np.linspace(0, length_beam, num_nodes)

    # Save individual mat files:
    filename = f"xB{building_offset}HL207.mat"
    print('------- SCRIPT RUNNING --------')


# %% Iteration control END #
        ...
        # %% INSTALLATION EFFECTS
            ...
            if avg_wall_disp_construction == 0:  # Do not allow values below 0
                ...
                if integration_mode == 'CValues':
                    Sx, Sz, Deltaw_all, VLW, Deltaw_max, check_ratio, VLS, ratiovol = wall_deflection(
    ...
    if input_type == 'TUNNEL':
        ...
        if output == 'quoFEM':
            ...
            with open("results.out", "w") as f:
                ...
                print('vector_length (Str7)  = ', len(dataReturn['tensile_strain_midpoint']),
                      ', vector_length (Str8)  = ',
                      len(dataReturn['strain_axial_bending_top_exx,b']), ', vector_length (Str9)  = ',
                      len(dataReturn['strain_axial_bending_bottom_exx,b']), ', vector_length (Str10)  = ',
                      len(dataReturn['tensile_strain_midpoint']))
                ...
                f.write("{} {} {} {} {} {} {} {} {} {} {} {} {} {} {}".format(Error_Variable,
                                                                              ...
                                                                              str6, str7, str8, str9, str10))
                print(' ')
                print('Iteration number = ', iterPara)  # Print iteration loop num
                print('--------- SCRIPT DONE ---------')
                print(' ')
                print(' ')
        else:
...
#  %% ITERATION OUTPUT %% #
    ...
    if makeIndividualMATfiles == True:
        data = {
            'model': {
                'type': "EL,  d_a = 18.0",
                'x_coordinate': building_coords,
                ...
                'xBoverHL': building_offset / retaining_wall_depth,
                'disp_beam_x': model_el.beam_DispL,
                'disp_beam_z': model_el.beam_DispV,
                'axialForce': model_el.axialForce,
                'shearForce': model_el.shearForce,
                'moment': model_el.moment,
                'eps_t_gf': dataSA['eps_t'],
                'eps_h_gf': dataSA['eps_h'],
                'beta_d': dataSA['beta_d'],
                'eps_t_SSI': dataReturn['eps_t'],
                'eps_t_mid': dataReturn['tensile_strain_midpoint'],
                'eps_t_top': dataReturn['strain_axial_bending_top_exx,b'],
                'eps_t_bottom': dataReturn['strain_axial_bending_bottom_exx,b'],
            }
        }
        scipy.io.savemat(filename, data)  # Saves individual data files

    elif makeIndividualMATfiles != False:
        print("Check state of var 'makeIndividualMATfiles', should be either True or False")

    result_LTS_Array[iterPara, :] = [iterPara, max_tensile_eps_gf, max_eps_t_ssi, max_tensile_eps_gf / max_eps_t_ssi,
                                     building_offset / retaining_wall_depth]

# %% ITERATION OUTPUT END %% #

dataLTS = {
    'results': {
        'type': 'EL',
        'x_coordinate': building_coords,
        'iterationVar': 'building_offset',
        'iteration_number': result_LTS_Array[:, 0],
        'eps_t_gf': result_LTS_Array[:, 1],
        'eps_t_SSI': result_LTS_Array[:, 2],
        'GFoverSSI': result_LTS_Array[:, 3],
        'normOffset': result_LTS_Array[:, 4],
    }
}

# Make final array
scipy.io.savemat(resultname, dataLTS)    # Saves data files
